...
/* -- MUSICA (fuente: YouTube) --
   El audio viene del video de YouTube configurado en YT_VIDEO_ID (ver seccion MUSIC PLAYER).
   Ya no necesitas archivos .mp3 locales. Para cambiar el video, edita YT_VIDEO_ID mas abajo. */
const TRACKS = [];
...
/* -- Cambia este ID si quieres usar otro video de YouTube -- */
const YT_VIDEO_ID = 'HuFYqnbVbzY';

const tracks   = [{ name: 'Lofi Hip Hop Radio', url: '', ytId: YT_VIDEO_ID }];
let currentIdx = 0;
let isPlaying  = false;
let isShuffle  = false;
let isLoop     = true;
...
// Inyectar contenedor oculto — necesita dimensiones minimas reales para que YT inicialice
(function injectYTContainer() {
  const div = document.createElement('div');
  div.id = 'yt-player-hidden';
  div.style.cssText = 'position:fixed;width:200px;height:113px;top:-9999px;left:-9999px;pointer-events:none;visibility:hidden;';
  document.body.appendChild(div);
})();

// Definir el callback ANTES de cargar el script de la API
window.onYouTubeIframeAPIReady = function () {
  const container = document.getElementById('yt-player-hidden');
  if (!container) { console.error('[YT] Contenedor no encontrado'); return; }
...
// Cargar la API de YouTube (despues de definir el callback)
(function loadYTApi() {
  ...
  tag.onerror = function () {
    showNotif('\u26A0 No se pudo cargar la API de YouTube');
  };
...
function onYTReady(e) {
  ...
  const savedVol = $('vol-slider') ? parseInt($('vol-slider').value) : 70;
  e.target.setVolume(savedVol);
  renderPlaylist();
  updateProgress();
  showNotif('\u266a Musica lista');
}
...
function onYTStateChange(e) {
  if (e.data === YT.PlayerState.PLAYING) {
    ...
    $('btn-play').textContent = '\u25B6';
    stopYTProgressTimer();
    if (isLoop && ytPlayer) ytPlayer.seekTo(0, true);
...
function onYTError(e) {
  ...
  showNotif('\u26A0 Error al cargar el video de YouTube');
  isPlaying = false;
  $('btn-play').textContent = '\u25B6';
}
...
function startYTProgressTimer() {
  stopYTProgressTimer();
  ytProgressTimer = setInterval(updateProgress, 1000);
}
...
function loadTrack(idx) {
  ...
  if (ytPlayer && ytReady) { ytPlayer.seekTo(0, true); if (isPlaying) ytPlayer.playVideo(); }
}

function playPause() {
  getAudioCtx();
  if (!ytPlayer || !ytReady) { showNotif('\u266a Cargando reproductor...'); return; }
  try {
    if (isPlaying) { ytPlayer.pauseVideo(); } else { ytPlayer.playVideo(); }
  } catch(err) {
    console.error('[YT] playPause error:', err);
    showNotif('\u26A0 Error al reproducir');
  }
}
...
function seekMusic(e) {
  if (!ytPlayer || !ytReady || !ytDuration) return;
...
function clickTrack() { playPause(); }

function renderPlaylist() {
  const pl = $('playlist');
  if (!pl) return;
  pl.innerHTML = `
    <div class="playlist-item ${isPlaying ? 'playing' : ''}" onclick="clickTrack(0)">
      <span class="dot"></span>
      <span class="track-name">\uD83C\uDFB5 ${escHtml(tracks[0].name)}</span>
    </div>`;
}
