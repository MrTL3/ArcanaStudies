...
const TRACKS = [
  ...
  // { name: 'Mi canción lofi',     url: 'assets/audio/music/cancion1.mp3' },
  // { name: 'Otra pista',          url: 'assets/audio/music/cancion2.mp3' },
];
...
/* ── ASISTENTE IA (Groq — gratuito, ultrarrápido) ──
   1. Ve a https://console.groq.com y crea una cuenta gratuita
   2. En "API Keys" genera una nueva key (no necesitas tarjeta)
   3. Pégala aquí entre las comillas
   Sin coste, sin límites prácticos para uso personal.              */
const GROQ_API_KEY = ''; // ej: 'gsk_xxxxxxxxxxxxxxxxxxxx'
...
/* ══════════════════════════════════════════════
   MUSIC PLAYER — Audio stream directo
   ══════════════════════════════════════════════ */

/* Streams de lofi disponibles — añade o cambia según prefieras */
const STREAMS = [
  {
    name: 'Lofi Hip Hop Radio',
    url:  'https://streams.ilovemusic.de/iloveradio17.mp3',  // lofi/chillhop stream
  },
  {
    name: 'Chillhop Radio',
...
let streamIdx    = 0;
let streamAudio  = new Audio();
let streamPlaying = false;
let streamMuted  = false;
let streamVol    = 70;

streamAudio.volume = streamVol / 100;
...
streamAudio.addEventListener('waiting', () => _stUpdateBtn('⏳'));
streamAudio.addEventListener('error',   () => {
  showNotif('⚠ No se pudo cargar el stream, prueba otro');
  streamPlaying = false;
  _stUpdateBtn();
  _stDotOff();
});

function _stUpdateBtn(override) {
  const btn = $('btn-play');
  if (!btn) return;
  btn.textContent = override || (streamPlaying ? '⏸' : '▶');
  // Sync fs clone
  const fsBtn = document.querySelector('#fs-music #btn-play');
  if (fsBtn) fsBtn.textContent = btn.textContent;
}
function _stDotOn()  {
  $('yt-track-dot')?.classList.add('playing');
  document.querySelector('#fs-music #yt-track-dot')?.classList.add('playing');
}
function _stDotOff() {
  $('yt-track-dot')?.classList.remove('playing');
  document.querySelector('#fs-music #yt-track-dot')?.classList.remove('playing');
}
function _stUpdateName() {
  const name = STREAMS[streamIdx]?.name || '';
  [$('yt-track-name'), document.querySelector('#fs-music #yt-track-name')]
...
function ytPlayPause() {
  if (streamPlaying) {
    ...
    streamAudio.src = STREAMS[streamIdx].url;
    streamAudio.load();
    streamAudio.play().catch(e => showNotif('⚠ ' + e.message));
  }
}

function ytSeekBack() {
  // Switch to previous stream
  streamIdx = (streamIdx - 1 + STREAMS.length) % STREAMS.length;
  _stSwitchStream();
  showNotif('♪ ' + STREAMS[streamIdx].name);
}
...
function _stSwitchStream() {
  const wasPlaying = streamPlaying;
  streamAudio.pause();
  streamAudio.src = STREAMS[streamIdx].url;
  _stUpdateName();
  if (wasPlaying) {
...
function ytToggleMute(btn) {
  streamMuted = !streamMuted;
  streamAudio.muted = streamMuted;
  const icon = streamMuted ? '🔇' : '🔊';
  if (btn) btn.textContent = icon;
  const fsBtn = document.querySelector('#fs-music #btn-mute');
  if (fsBtn) fsBtn.textContent = icon;
}

function ytSetVolume(val) {
  streamVol = parseInt(val);
  streamAudio.volume = streamVol / 100;
  streamAudio.muted = false;
  streamMuted = false;
  [$('btn-mute'), document.querySelector('#fs-music #btn-mute')]
    .forEach(b => { if (b) b.textContent = '🔊'; });
}

function ytSeek() { /* streams en vivo — no seekable */ }

// Progress bar — live streams no tienen duración, mostrar tiempo de escucha
let streamListenTime = 0;
setInterval(() => {
  if (!streamPlaying) return;
  streamListenTime++;
  const tc = fmtTime(streamListenTime);
  [$('time-current'), document.querySelector('#fs-music #time-current')]
    .forEach(el => { if (el) el.textContent = tc; });
  [$('time-total'), document.querySelector('#fs-music #time-total')]
    .forEach(el => { if (el) el.textContent = '∞'; });
  const pct = Math.min(100, (streamListenTime % 300) / 300 * 100);
  [$('progress-bar-fill'), document.querySelector('#fs-music #progress-bar-fill')]
    .forEach(el => { if (el) el.style.width = pct + '%'; });
}, 1000);

// Stubs legacy
function playPause()    { ytPlayPause(); }
function setVolume(val) { ytSetVolume(val); }
function renderPlaylist() {}

// Init — show first stream name
_stUpdateName();
...
function syncFsState() {
  ...
  if (fsMusic) {
    ...
    if (playBtn) playBtn.textContent = ytPlaying ? '⏸' : '▶';
    // Sync mute button
    const muteBtn = fsMusic.querySelector('#btn-mute');
    if (muteBtn) muteBtn.textContent = ytMuted ? '🔇' : '🔊';
    // Sync volume slider
    const volSlider = fsMusic.querySelector('#volume');
    if (volSlider) volSlider.value = ytVolume;
    // Progress is synced directly in _ytUpdateProgress every second
  }
...
async function sendChatMsg() {
  if (!GROQ_API_KEY) {
    addChatBubble('assistant',
      ...
      '4. Pégala en app.js → GROQ_API_KEY');
    return;
  }
  ...
  try {
    ...
    if (data.error) {
      addChatBubble('assistant', '⚠ Error: ' + (data.error.message || JSON.stringify(data.error)));
    } else {
    ...
    typing.remove();
    addChatBubble('assistant', '⚠ No se pudo conectar. Comprueba tu conexión y la API key de Groq.');
  }
